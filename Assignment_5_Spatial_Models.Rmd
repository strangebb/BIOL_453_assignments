...
The results of both tests look relatively similar. Both indicate that observed correlation is not significantly greater than null correlation. As a result, the distribution of Tachigali versicolor is not spatially dependent but instead appears to be random. There does not appear to be evidence of spatial dependence. 
...
#plot results
plot(xy_dist, standleyi_dist, main = "Drypetes standleyi spatial dependence")
abline(lm(standleyi_dist ~ xy_dist), lwd=3, col='red')
lines(lowess(xy_dist, standleyi_dist), lwd=3, col='pink')
abline(v = max_dist, col='red', lwd=3, lty=2)
plot(xy_dist, lasiocalyx_dist, main = "Cordia lasiocalyx spatial dependence")
...
tree_dat = data.frame(BCI$Drypetes.standleyi, BCI$Cordia.lasiocalyx, BCI, BCI_xy)
standleyi_1var_lm = gls(Drypetes.standleyi ~ Cordia.lasiocalyx, data = tree_dat)

#use Variogram() to examine spatial dependence of residuals
par(mfrow=c(1,1))
plot(Variogram(standleyi_1var_lm, form= ~ x + y), main = "standleyi 1 var")
res = residuals(standleyi_1var_lm)
plot(dist(tree_dat[, c('x', 'y')]), dist(res), main = "standleyi 1 var residual spatial dependence")
...
plot(Variogram(standleyi_1var_exp, maxDist = max_dist), main = "standleyi 1 var exp")

#update plot to examine normalized residuals
plot(Variogram(standleyi_1var_exp, resType='normalized', maxDist = max_dist), main = "standleyi 1 var exp normalized")

#update model to include nugget 
standleyi_1var_exp_nug = update(standleyi_1var_exp, corr=corExp(form=~x + y, nugget=T))
plot(Variogram(standleyi_1var_exp_nug, maxDist = max_dist), main = "standleyi 1 var exp nugget")

#update plot with normalized residuals
plot(Variogram(standleyi_1var_exp_nug, resType='n', maxDist = max_dist), main = "standleyi 1 var exp nugget normalized")
```
...
plot(Variogram(standleyi_1var_rat, maxDist = max_dist), main = "standleyi 1 var rational quad")

#update plot with normalized distribution
plot(Variogram(standleyi_1var_rat, resType='n', maxDist = max_dist), main = "standleyi 1 var rational quad normalized")

#update model to include nugget
standleyi_1var_rat_nug = update(standleyi_1var_lm, corr=corRatio(form=~x + y, nugget=T))
plot(Variogram(standleyi_1var_rat_nug, maxDist = max_dist), main = "standleyi 1 var rational quad nugget")

#update plot with normalized distribution
plot(Variogram(standleyi_1var_rat_nug, resType='n', maxDist = max_dist), main = "standleyi 1 var rational quad nugget normalized")
```
...
tree_dat = data.frame(BCI$Drypetes.standleyi, BCI$Cordia.lasiocalyx, BCI$Hirtella.triandra, BCI$Picramnia.latifolia, BCI$Quassia.amara, BCI$Tabernaemontana.arborea, BCI$Trattinnickia.aspera, BCI$Xylopia.macrantha, BCI, BCI_xy)
standleyi_multvar_lm = gls(Drypetes.standleyi ~ Cordia.lasiocalyx + Hirtella.triandra + Picramnia.latifolia + Quassia.amara + Tabernaemontana.arborea + Trattinnickia.aspera + Xylopia.macrantha, data = tree_dat)

#use Variogram() to examine spatial dependence of residuals
par(mfrow=c(1,1))
plot(Variogram(standleyi_multvar_lm), main = "standleyi Multvar Mod")
res2 = residuals(standleyi_multvar_lm)
plot(dist(tree_dat[, c('x', 'y')]), dist(res2), main = "standleyi multvar residual spatial dependence")
lines(lowess(dist(tree_dat[, c('x', 'y')]), dist(res2)), col='red', lwd=2)
abline(v = max_dist, col='red', lwd=3, lty=2)
```
...
plot(Variogram(standleyi_multvar_exp, maxDist = max_dist), main = "standleyi multvar exp")

#update plot to examine normalized residuals
plot(Variogram(standleyi_multvar_exp, resType='normalized', maxDist = max_dist), main = "standleyi multvar exp normalized")

#update model to include nugget 
standleyi_multvar_exp_nug = update(standleyi_multvar_exp, corr=corExp(form=~x + y, nugget=T))
plot(Variogram(standleyi_multvar_exp_nug, maxDist = max_dist), main = "standleyi multvar exp nugget")

#update plot with normalized residuals
plot(Variogram(standleyi_multvar_exp_nug, resType='n', maxDist = max_dist), main = "standleyi multvar exp nugget normalized")
```
...
plot(Variogram(standleyi_multvar_rat, maxDist = max_dist), main = "standleyi multvar rational quad")

#update plot with normalized distribution
plot(Variogram(standleyi_multvar_rat, resType='n', maxDist = max_dist), main = "standleyi multvar rational quad normalized")

#update model to include nugget
standleyi_multvar_rat_nug = update(standleyi_multvar_lm, corr=corRatio(form=~x + y, nugget=T))
plot(Variogram(standleyi_multvar_rat_nug, maxDist = max_dist), main = "standleyi multivar rational quad nugget")

#update plot with normalized distribution
plot(Variogram(standleyi_multvar_rat_nug, resType='n', maxDist = max_dist), main = "standleyi multivar rational quad nugget normalized")
```
...
The inclusion of the exponential error term and the rational quadratic error term had the largest effect on the model coefficient for Quassia amara (went from ~4 in the original model to ~1-2 in the other models) and on the intercept.
